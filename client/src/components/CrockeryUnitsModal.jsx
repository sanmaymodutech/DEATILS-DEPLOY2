...
const CrockeryUnitsModal = ({
  ...
  useEffect(() => {
    if (open) {
      if (crockeryUnitIndex === null || crockeryUnitIndex === undefined) {
        setError("Crockery unit index is not defined. Please try again.");
        return;
      }

      resetForm();

      // Fetch existing units for this crockery unit
      const fetchUnits = async () => {
        try {
          const response = await api.get(
            `/quotations/${quotationId}/rooms/${roomIndex}/crockeryUnit/${crockeryUnitIndex}`
          );
          if (response.data.data && response.data.data.units) {
            setUnits(response.data.data.units);
          }
        } catch (error) {
          console.error("Failed to fetch crockery unit units:", error);
  ...
  }, [open, crockeryUnitIndex, quotationId, roomIndex]);
  ...
  const handleAddUnit = async () => {
    if (crockeryUnitIndex === null || crockeryUnitIndex === undefined) {
      setError(
        "Crockery unit index is not defined. Please close and try again."
      );
      return;
    }
    if (!validateInputs()) {
    ...
    try {
      ...
      const response = await api.put(
        `/quotations/${quotationId}/rooms/${roomIndex}/crockeryUnit/${crockeryUnitIndex}/units`,
        payload
      );
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>Define Crockery Unit Components</DialogTitle>
      <DialogContent>
        {error && (
...
export default CrockeryUnitsModal;
