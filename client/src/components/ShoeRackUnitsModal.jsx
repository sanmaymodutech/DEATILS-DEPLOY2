...
const ShoeRackUnitsModal = ({
  ...
  useEffect(() => {
    if (open) {
      if (shoeRackIndex === null || shoeRackIndex === undefined) {
        setError("Shoe Rack index is not defined. Please try again.");
        return;
      }

      resetForm();

      // Fetch existing units for this shoe rack
      const fetchUnits = async () => {
        try {
          const response = await api.get(
            `/quotations/${quotationId}/rooms/${roomIndex}/shoeRack/${shoeRackIndex}`
          );
          if (response.data.data && response.data.data.units) {
            setUnits(response.data.data.units);
          }
        } catch (error) {
          console.error("Failed to fetch shoe rack units:", error);
  ...
  }, [open, shoeRackIndex, quotationId, roomIndex]);
  ...
  const handleAddUnit = async () => {
    if (shoeRackIndex === null || shoeRackIndex === undefined) {
      setError("Shoe Rack index is not defined. Please close and try again.");
      return;
    }
    if (!validateInputs()) {
      return;
    ...
    try {
      ...
      const response = await api.put(
        `/quotations/${quotationId}/rooms/${roomIndex}/shoeRack/${shoeRackIndex}/units`,
        payload
      );
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>Define Shoe Rack Units</DialogTitle>
      <DialogContent>
        {error && (
            ...
            <>
              ...
              <Grid item xs={6} md={3}>
                <TextField
                  ...
                  fullWidth
                  value={drawerWeight}
                  onChange={(e) => setDrawerWeight(e.target.value)}
                  size="small"
                >
                  {drawerWeights.map((weight) => (
...
export default ShoeRackUnitsModal;
