...
const ConsoleUnitsModal = ({
  ...
  useEffect(() => {
    if (open) {
      if (consoleUnitIndex === null || consoleUnitIndex === undefined) {
        setError("Console Unit index is not defined. Please try again.");
        return;
      }

      resetForm();

      // Fetch existing units for this console unit
      const fetchUnits = async () => {
        try {
          const response = await api.get(
            `/quotations/${quotationId}/rooms/${roomIndex}/consoleUnit/${consoleUnitIndex}`
          );
          if (response.data.data && response.data.data.units) {
            setUnits(response.data.data.units);
          }
        } catch (error) {
          console.error("Failed to fetch console units:", error);
  ...
  }, [open, consoleUnitIndex, quotationId, roomIndex]);
  ...
  const handleAddUnit = async () => {
    if (consoleUnitIndex === null || consoleUnitIndex === undefined) {
      setError(
        "Console Unit index is not defined. Please close and try again."
      );
      return;
    }
    if (!validateInputs()) {
    ...
    try {
      ...
      const response = await api.put(
        `/quotations/${quotationId}/rooms/${roomIndex}/consoleUnit/${consoleUnitIndex}/units`,
        payload
      );
  ...
  const handleRemoveUnit = async (index) => {
    try {
      // Since API doesn't have delete endpoint for units,
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>Define Console Unit Components</DialogTitle>
      <DialogContent>
        {error && (
...
export default ConsoleUnitsModal;
