class TVUnitCalculator {
  static PROFILE_SHUTTER_RATE = 1100;
  static TV_PANEL_RATE = 500;
  static calculateSquareFeet(width, height) {
    const sqft = (width * height) / 92900;
    return Math.round(sqft * 100) / 100;
  }

  static async calculateFillerPrice(measurements, config, prices) {
    const { width, height, depth } = measurements;
    const { shutterMaterial, shutterType, finish = "WHITE" } = config;

    // Basic validations
    if (width > 150) {
      throw new Error("Filler width must not exceed 150mm");
    }

    const sqft = this.calculateSquareFeet(width, height);

    // Reuse existing shutter price calculation logic
    const shutterPrice =
      shutterMaterial === "HDHMR"
        ? this.getShutterPrice(shutterMaterial, finish, shutterType, prices)
        : this.getShutterPrice(shutterMaterial, finish, null, prices);

    const totalShutterCost = Math.round(shutterPrice * sqft);

    return {
      squareFeet: sqft,
      shutterPrice: shutterPrice,
      shutterCost: totalShutterCost,
      ratePerSqFt: { shutter: shutterPrice },
      total: totalShutterCost,
    };
  }
  ...
  static getCarcassPrice(type, finish, prices) {
    if (!prices?.carcass?.[type]?.[finish.toLowerCase()]) {
      throw new Error(
        `Invalid carcass pricing for type: ${type} and finish: ${finish}`
      );
    }
  ...
  static getShutterPrice(material, finish, shutterType, prices) {
    if (material === "HDHMR") {
      if (!prices?.shutters?.HDHMR?.[shutterType]?.[finish.toLowerCase()]) {
        throw new Error(
          `Invalid HDHMR shutter pricing for type: ${shutterType} and finish: ${finish}`
        );
      }
    ...
    if (!prices?.shutters?.[material]?.[finish.toLowerCase()]) {
      throw new Error(
        `Invalid shutter pricing for material: ${material} and finish: ${finish}`
      );
    }
  ...
  static getProfileShutterPrice(material, shutterType, finish, prices) {
    if (material === "GLASS_PROFILE") {
      if (
        !prices?.shutters?.[material]?.[shutterType]?.[finish.toLowerCase()]
      ) {
        throw new Error(
          `Invalid profile shutter pricing for material: ${material}, type: ${shutterType} and finish: ${finish}`
        );
      }
  ...
  static getOpenUnitPrice(material, shutterType, prices) {
    ...
    if (material === "HDHMR") {
      if (!prices?.shutters?.HDHMR?.[shutterType]) {
        throw new Error(
          `Invalid HDHMR shutter pricing for type: ${shutterType}`
        );
      }
  ...
  static getDrawerPrice(width, weight = "30KG", prices) {
    ...
    if (!prices.tandemDrawers[widthBracket]?.[weight]) {
      throw new Error(
        `Invalid drawer pricing for width: ${widthBracket} and weight: ${weight}`
      );
    }
  ...
  static async calculateTVUnitPrice(measurements, config, prices) {
    try {
      const { width, height, depth } = measurements;
      const {
        unitType,
      ...
      switch (unitType) {
        case "CARCASS_WITH_SHUTTERS": {
          ...
          const carcassPrice = this.getCarcassPrice(
            carcassType,
            finish,
            prices
          );
          const shutterPrice = this.getShutterPrice(
            shutterMaterial,
            finish,
            shutterType,
            prices
        ...
        case "CARCASS_WITH_PROFILE_SHUTTER": {
          if (!carcassType) {
            throw new Error(
              "Carcass type is required for carcass with profile shutter units"
            );
          }
          ...
          if (shutterMaterial && shutterType) {
            profileShutterPrice = this.getProfileShutterPrice(
              shutterMaterial,
              shutterType,
              finish,
              prices
        ...
        case "OPEN_UNIT": {
          ...
          const openUnitPrice = this.getOpenUnitPrice(
            shutterMaterial,
            shutterType,
            prices
          );
          ...
          priceBreakdown.baseShutterPrice = Math.round(
            (openUnitPrice / 1.25) * sqft
          );
          priceBreakdown.markup = Math.round(
            openUnitPrice * sqft - (openUnitPrice / 1.25) * sqft
          );

          totalCost = priceBreakdown.openUnit;

          // Calculate shelves if required
          if (shelvesRequired && shelvesQuantity > 0) {
            // Calculate shelf square feet (width x depth)
            const shelfSqft = this.calculateSquareFeet(width, depth);

            // Get the base shutter price (same as used for open unit calculation)
            let baseShutterPrice;
            if (shutterMaterial === "HDHMR") {
              baseShutterPrice =
                prices.shutters.HDHMR[shutterType][finish.toLowerCase()];
            } else {
              baseShutterPrice =
                prices.shutters[shutterMaterial][finish.toLowerCase()];
            }

            // Apply 25% markup to shutter price for shelves (same as open unit)
            const shelfRate = baseShutterPrice * 1.25;

            // Calculate cost for all shelves
        ...
        case "TV_PANEL": {
          // Calculate the cost based on square feet and the TV panel rate
          const tvPanelCost = Math.round(this.TV_PANEL_RATE * sqft);

          priceBreakdown.tvPanel = tvPanelCost;
          priceBreakdown.ratePerSqFt = { tvPanel: this.TV_PANEL_RATE };
          totalCost = tvPanelCost;
        ...
        case "DRAWER": {
          ...
          const drawerMechanismPrice = this.getDrawerPrice(
            width,
            drawerWeight,
            prices
          );
      ...
      throw new Error(`TV unit price calculation failed: ${error.message}`);
    }
  }
}

module.exports = TVUnitCalculator;
